...
def generate_bottleneck_graph(params: BottleneckGraphParams) -> nx.Graph:
    ...
    for _ in range(params.nodes_per_side):
        pos = (
            -params.side_distance + rng.uniform(-params.bubble_radius, params.bubble_radius),
            rng.uniform(-params.bubble_radius, params.bubble_radius),
            rng.uniform(-params.bubble_radius, params.bubble_radius),
        )
    ...
    for i in range(params.bridge_nodes):
        ...
        G.add_node(f"bridge_{i}", reward=rng.randint(*params.reward_range), pos=pos)
    ...
    for _ in range(params.nodes_per_side):
        ...
        reward = int(rng.randint(*params.reward_range) * params.far_reward_multiplier)
