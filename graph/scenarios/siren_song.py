...
def generate_siren_song_graph(
    ...
    idx = 1
    for _ in range(n_local_nodes):
        phi = random.uniform(0, 2 * math.pi)
        theta = random.uniform(0, math.pi)
        r = random.uniform(*local_dist_range)
        ...
        G.add_node(f"s{idx}", reward=random.randint(*local_reward_range), pos=pos)
        idx += 1

    siren_pos = (siren_distance, 0.0, 0.0)
    G.add_node("SIREN", reward=siren_reward, pos=siren_pos)
