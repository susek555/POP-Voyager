...
def generate_nebula_graph(
    ...
    for i in range(1, n_nodes + 1):
        ...
        G.add_node(f"s{i}", reward=random.randint(*reward_range), pos=pos)
    ...
    for i in range(len(nodes)):
        for j in range(i + 1, len(nodes)):
            ...
            base_cost = calc_cost(data_u["pos"], data_v["pos"], cost_factor)
            ...
            dist_to_nebula = math.sqrt(
                (midpoint[0] - nebula_center[0])**2 +
                (midpoint[1] - nebula_center[1])**2 +
                (midpoint[2] - nebula_center[2])**2
            )

            final_cost = base_cost
            if dist_to_nebula < nebula_radius:
                final_cost *= nebula_multiplier
